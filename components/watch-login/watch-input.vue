<template>
	<view class="main-list oBorder">
		<!-- 图标 -->
		<image :src="icon" style="width: 28rpx;height: 38rpx;margin-right:15rpx" v-if="icon"></image>
		<!-- 文本框 -->
		<input class="main-input" :value="value" :type="_type" :maxlength="maxlength" :placeholder="placeholder"
			:password="type==='password'&&!showPassword" @input="onInput"  :data-key="dataKey" @confirm="toLogin"/>
		...
		<view v-if="_isShowGet" class="vercode" @click="setNumberCode">官方邀请码</view>
	...
	export default {
		...
		methods: {
			...
			toLogin(e){
				...
				this.$emit('toLogin', e.target.value)
			},
			setCode() {
				//设置获取验证码的事件
				if (this.isRunCode) {
