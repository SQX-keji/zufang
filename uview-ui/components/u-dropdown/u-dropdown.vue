			...
			<view class="u-dropdown__menu__item" v-for="(item, index) in menuList" :key="index"
				...
				<view class="u-flex">
					<text class="u-dropdown__menu__item__text" :style="{
						...
						fontSize: $u.addUnit(titleSize),fontWeight: item.disabled ? '#c0c4cc' : (index === current || highlightIndex == index) ? 800 : 400,
					}">{{item.title}}</text>
					<view class="u-dropdown__menu__item__arrow" :class="{
						'u-dropdown__menu__item__arrow--rotate': index === current
					}">
						<u-icon :custom-style="{display: 'flex'}" :name="menuIcon" :size="$u.addUnit(menuIconSize)"
							:color="index === current || highlightIndex == index ? activeColor : '#333333'"></u-icon>
					</view>
				</view>
			</view>
		</view>
